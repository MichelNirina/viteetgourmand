...
async function load() {
    ...
    app.innerHTML = `
        ...
        <ul>
            ${items.map(i => `<li>${esc(i.libelle)} <button class="del" data-id="${esc(i.regime_id)}">✕</button></li>`).join('')}
        </ul>
    `;
    ...
    document.getElementById('form').addEventListener('submit', async (e) => {
        e.preventDefault();
        try {
            await createRegime(Object.fromEntries(new FormData(e.target)));
            load();
        } catch (ex) { alert(ex.message); }
    });

    document.querySelectorAll('.del').forEach(b => b.addEventListener('click', async () => {
        try { await deleteRegime(b.dataset.id); load(); } catch (ex) { alert(ex.message); }
    }));
}
